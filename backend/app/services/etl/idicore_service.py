...
class IdiCOREAPIService:
    ...
    def _ensure_snowflake_cache(self):
        ...
        if self.snowflake_cache is None:
            try:
                ...
                self.logger.warning(f"Could not initialize Snowflake cache: {e}. Cache operations will be skipped.")
    ...
    def lookup_person_phones_and_emails(self, first_name: str, last_name: str, address: str, 
    ...
    def _lookup_single_person_without_caching(self, person: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"❌ Error processing individual person {person.get('first_name', '')} {person.get('last_name', '')}: {e}")
            return {'phones': [], 'emails': [], 'display_phone': '', 'status': 'error'}
    
    def _lookup_uncached_people_threaded(self, people_data: List[Dict[str, str]], max_workers: int = 150) -> List[Dict[str, Any]]:
        """Look up phone numbers for uncached people with parallel individual requests"""
        results = []
        total_people = len(people_data)
        
        self.logger.info(f"🚀 Starting parallel API calls for {total_people} uncached people using {max_workers} threads")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    self.logger.error(f"❌ Error processing individual request for {person.get('first_name', '')} {person.get('last_name', '')}: {e}")
                    results[index] = {'phones': [], 'emails': [], 'display_phone': '', 'status': 'error'}
        ...
        self.logger.info(f"✅ Parallel API calls completed: {len(results)} people, {found_phones} phones found")
