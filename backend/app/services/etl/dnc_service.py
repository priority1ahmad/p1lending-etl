...
class DNCCheckerDB:
    ...
    def _verify_database(self) -> bool:
        ...
        if not os.path.exists(self.db_path):
            self.logger.warning(
                f"DNC database not found at {self.db_path}. DNC checking will be disabled."
            )
            self.logger.warning(
                "Please ensure the DNC database file exists at one of these locations:"
            )
            self.logger.warning(
                "  - /app/dnc_database.db (Docker mount - recommended, from /home/ubuntu/etl_app/dnc_database.db)"
            )
            self.logger.warning("  - /app/data/dnc_database.db (Docker volume)")
            self.logger.warning("  - /home/ubuntu/etl_app/dnc_database.db (Production host)")
        ...
        try:
            ...
            if table_exists:
                ...
                self.logger.warning(
                    "DNC database exists but 'dnc_list' table not found. DNC checking will be disabled."
                )
                return False
    ...
    def check_single_phone(self, phone: str) -> Dict:
        ...
        try:
            ...
            return {"phone": phone, "in_dnc_list": False, "status": "error", "error": str(e)}
    ...
    def check_multiple_phones(self, phones: List[str]) -> List[Dict]:
        ...
        if not os.path.exists(self.db_path):
            self.logger.warning(
                f"DNC database not found at {self.db_path}. Returning empty results."
            )
            return [
                {
        ...
        self.logger.info(
            f"🔍 Checking {total_phones} phone numbers against DNC database (batched query)"
        )
        ...
        try:
            ...
            if not normalized_phones:
                self.logger.warning(
                    f"No valid phone numbers to check (all {total_phones} phones invalid)"
                )
                return [
                    {
                        "phone": phone_str,
            ...
            for chunk_idx, chunk in enumerate(chunks):
                ...
                try:
                    ...
                    if len(chunks) > 1:
                        self.logger.debug(
                            f"DNC chunk {chunk_idx + 1}/{len(chunks)}: {len(chunk)} phones, {len(chunk_results)} matches"
                        )
                except Exception as e:
            ...
            self.logger.info(
                f"✅ DNC check completed (batched): {in_dnc_count}/{total_phones} phones found in DNC list ({success_count} successful checks)"
            )
