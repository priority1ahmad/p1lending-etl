...
def upgrade() -> None:
    ...
    op.add_column(
        'etl_jobs',
        sa.Column('file_source_id', UUID(as_uuid=True), sa.ForeignKey('file_sources.id', ondelete='SET NULL'), nullable=True)
    )

    # Add file_upload_id column to etl_jobs
    op.add_column(
        'etl_jobs',
        sa.Column('file_upload_id', UUID(as_uuid=True), sa.ForeignKey('file_uploads.id', ondelete='SET NULL'), nullable=True)
    )
