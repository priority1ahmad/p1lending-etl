...
def upgrade() -> None:
    ...
    op.create_table(
        'etl_jobs',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('job_type', sa.Enum('single_script', 'all_scripts', name='jobtype'), nullable=False),
        sa.Column('script_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('status', sa.Enum('pending', 'running', 'completed', 'failed', 'cancelled', name='jobstatus'), nullable=False, default='pending'),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.ForeignKeyConstraint(['script_id'], ['sql_scripts.id'], ),
