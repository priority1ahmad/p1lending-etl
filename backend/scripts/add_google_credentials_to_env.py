...
def add_credentials_to_env(credentials_path: str, env_path: str = None):
    ...
    print("The backend will automatically use these credentials from the environment variable.")
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python add_google_credentials_to_env.py <path_to_google_credentials.json> [path_to_env_file]"
        )
        print("\nExample:")
        print(
            "  python backend/scripts/add_google_credentials_to_env.py old_app/google_credentials.json"
        )
        sys.exit(1)
